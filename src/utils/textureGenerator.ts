...
// ----------------------------------------------------
// LEVEL 3: REACTOR CORE TEXTURES
// ----------------------------------------------------

/** Heavy industrial diamond plate / grating floor with hazard stripes */
export function getReactorFloorTexture(): THREE.CanvasTexture {
  return createTexture('reactor_floor', 512, 512, 8, 8, (ctx, w, h) => {
    // Dark oxidized rust metal
    ctx.fillStyle = '#1c1917';
    ctx.fillRect(0, 0, w, h);

    // Industrial tread diamond pattern
    ctx.fillStyle = '#44403c';
    const step = 32;
    for (let x = 0; x < w; x += step) {
      for (let y = 0; y < h; y += step) {
        ctx.beginPath();
        ctx.ellipse(x + 10, y + 10, 8, 3, Math.PI / 4, 0, Math.PI * 2);
        ctx.fill();

        ctx.beginPath();
        ctx.ellipse(x + 24, y + 24, 8, 3, -Math.PI / 4, 0, Math.PI * 2);
        ctx.fill();
      }
    }

    // Red radioactive glowing seam lines
    ctx.strokeStyle = 'rgba(239, 68, 68, 0.4)';
    ctx.lineWidth = 3;
    ctx.strokeRect(0, 0, w, h);

    // Cracked glow veins radiating from a corner, like heat-stressed plating
    ctx.strokeStyle = 'rgba(251, 146, 60, 0.35)';
    ctx.lineWidth = 1.5;
    for (let v = 0; v < 4; v += 1) {
      ctx.beginPath();
      ctx.moveTo(0, 0);
      const angle = (v / 4) * (Math.PI / 2.4);
      ctx.lineTo(Math.cos(angle) * w * 0.9, Math.sin(angle) * h * 0.9);
      ctx.stroke();
    }

    addGrime(ctx, w, h, 260, '#000000', 131);
    addGrime(ctx, w, h, 50, '#f97316', 149);
  });
}

/** Heavy reinforced blast wall plating with caution stripes and hazard symbols */
export function getReactorWallTexture(): THREE.CanvasTexture {
  return createTexture('reactor_wall', 512, 512, 6, 2, (ctx, w, h) => {
    // Dark heavy blast plating
    ctx.fillStyle = '#1c1917';
    ctx.fillRect(0, 0, w, h);
    ...
    for (let p = 0; p < 2; p++) {
      ...
      ctx.lineWidth = 3;
      ctx.strokeRect(px + 12, 12, pw - 24, h - 24);

      // Warning hazard diagonal stripes (Yellow/Black)
      const barY = h - 60;
      const barH = 30;
      ctx.save();
      ctx.beginPath();
      ctx.rect(px + 16, barY, pw - 32, barH);
      ctx.clip();

      ctx.fillStyle = '#eab308';
      ctx.fillRect(px + 16, barY, pw - 32, barH);

      ctx.fillStyle = '#0c0a09';
      for (let s = -50; s < pw + 50; s += 24) {
        ctx.beginPath();
        ctx.moveTo(px + s, barY);
        ctx.lineTo(px + s + 12, barY);
        ctx.lineTo(px + s - 8, barY + barH);
        ctx.lineTo(px + s - 20, barY + barH);
        ctx.closePath();
        ctx.fill();
      }
      ctx.restore();
      ...
      ctx.fillText('SECTOR_03 // CRITICAL', px + 24, 60);
      ...
      // Rust/scorch streaks bleeding down from the top seam
      ...
      for (let r = 0; r < 3; r += 1) {
        ...
        ctx.beginPath();
        ctx.moveTo(rx, 12);
        ctx.quadraticCurveTo(rx + 6, h * 0.4, rx - 4, h * 0.7);
        ctx.stroke();
      }
    }

    addGrime(ctx, w, h, 180, '#000000', 163);
  });
}
